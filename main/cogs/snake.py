...
class snake(commands.Cog):
    def __init__(self, bot: commands.Bot):
        self.bot = bot
        self.invites = {}
        #lobbyname: {boardpos: "", black:"", white:""}
        self.current_games = {}
    
    @app_commands.command(name="snake", description="Request or Accept a game of snake with somebody")
    async def snake(self, interaction, p2 : discord.User):
        p1 = discord.User(interaction.user)
        self.current_games[f"{p1.name}{p2.name}"] = {"board": loadposfromfen(boardCmds)}
        await interaction.response.send_message(self.current_games[f"{p1.name}{p2.name}"]["board"])
